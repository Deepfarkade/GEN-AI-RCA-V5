    ...
    def get_sql_prompt(
        ...
        **kwargs,
        ...
        if initial_prompt is None:
            ...
            "Generate a SQL query to answer the question based strictly on the provided **CONTEXT** (DDL(Tables), Additional context(Documentations), Relations between the tables and Question SQL pairs ) and **HISTORY** (previous interactions). First, determine if the question is a **follow-up** (build on prior queries) or a **new question** (standalone query), and craft the query accordingly. Ensure your response adheres to the **response guidelines**
        ...
        initial_prompt += (
            f"\n\n  [HISTORY] : ****PREVIOUS 3 QUESTIONS ASKED BY USER are**** \n {context_str} =========="
        )
